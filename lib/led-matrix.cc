...
RGBMatrix *RGBMatrix::CreateFromOptions(const RGBMatrix::Options &options,
  ...
  const int max_gpio_slowdown = 250;
  ...
  if (runtime_options.do_gpio_init && !use_rp1_backend
      && !io.Init(runtime_options.gpio_slowdown, runtime_options.opi_delay_ns)) {
